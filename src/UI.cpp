...
void UI::addMixControls()
{
	...
	buttonSelect.push_back(mixParams->addButton(toString(i), std::bind(&UI::setPreview, this, i, std::placeholders::_1), "{ \"width\":48, \"stateless\":false, \"group\":\"pvw\", \"exclusive\":true }"));
	// does not work sliderCrossfade.push_back(mixParams->addSlider("xFade", &mTextures->warpInputs[i].iCrossfade, "{ \"min\":0.0, \"max\":1.0, \"width\":96 }"));
}
